...
def check_color(hsv, color):
    if (color[0][0] <= hsv[0] <= color[1][0]) and (color[0][1] <= hsv[1] <= color[1][1]) and \
...
def eye_color(image):
    ...
    if result == []:
        st.warning('Warning: Can not detect any face in the input image!')
        return
    ...
    cv2.circle(img_mask, left_eye, int(eye_radius), (255, 255, 255), -1)
    cv2.circle(img_mask, right_eye, int(eye_radius), (255, 255, 255), -1)
    ...
    for y in range(0, h):
        for x in range(0, w):
            if img_mask[y, x] != 0:
                eye_class[find_class(img_hsv[y, x])] += 1
    ...
    for i in range(len(class_name)):
        st.write(class_name[i], ": ", round(eye_class[i] / total_vote * 100, 2), "%")
...
def main():
    ...
    if input_type == "Image":
        ...
        st.warning("Video input is not yet supported in this version.")
